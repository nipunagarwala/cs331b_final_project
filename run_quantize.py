	...
	for i, (data, _) in enumerate(test_loader):
		...
		if i == 0:
			...
			save_image(comparison.data.cpu(),
						'/diskhdd/cs331b/results/simple_vae/quantize_stoch_tuned_4_' + str(epoch) + '.png', nrow=8)
...
def tune_model(args):
	...
	for epoch in range(1, num_epochs + 1):
		...
		torch.save(curModel.state_dict(), '/diskhdd/cs331b/checkpoints/simple_vae_ckpt/quant_ckpt_stoch_tuned-' + str(epoch) + '.pt')
...
def quantize_and_test(args):
	...
	train_loader, test_loader = load_datasets(batch_size)
	run_test_epoch('quant', quantModel, test_loader, batch_size)
